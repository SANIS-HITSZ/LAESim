...
#----------------------------------- Multi-Vehicle Client (UAV + UGV) ---------------------------------------------
class MultiVehicleClient(object):
    """
    Unified client for scenarios with both drones (UAV) and cars (UGV).
    Automatically routes API calls to the correct RPC port (41471 for Multirotor, 41461 for Car).
    ...
    def simGetVehiclePose(self, vehicle_name=''):
        """Get pose - tries Multirotor (41471) then Car (41461) port."""
        return self._try_multirotor_then_car(
            self._multirotor.simGetVehiclePose,
            self._car.simGetVehiclePose,
            vehicle_name
        )
    def getLidarData(self, lidar_name='', vehicle_name=''):
        """Get lidar data - tries Multirotor (41471) then Car (41461) port."""
        try:
            return self._multirotor.getLidarData(lidar_name, vehicle_name)
        except Exception:
            return self._car.getLidarData(lidar_name, vehicle_name)
    def simGetImages(self, requests, vehicle_name='', external=False):
        """Get images - tries Multirotor (41471) then Car (41461) port."""
        try:
    ...
    def enableApiControl(self, is_enabled, vehicle_name=''):
        """Enable API control - tries both clients."""
        try:
    ...
    def listVehicles(self):
        """List all vehicles. Returns names from multirotor server (drone names)."""
        try:
            multi = set(self._multirotor.listVehicles())
            car = set(self._car.listVehicles())
            return list(multi | car)
        except Exception:
