...
#----------------------------------- Boat APIs ---------------------------------------------
class BoatClient(VehicleClient, object):
    def __init__(self, ip = "", port = RPCLIB_PORT_BOAT, timeout_value = 3600):
        super(BoatClient, self).__init__(ip, port, timeout_value)
    def setBoatControls(self, controls, vehicle_name = ''):
        """
        Control the boat using throttle, steering, brake, and anchor.
        Args:
            controls (BoatControls): Struct containing control values
            vehicle_name (str, optional): Name of vehicle to be controlled
        """
        self.client.call('setBoatControls', controls, vehicle_name)
    def getBoatState(self, vehicle_name = ''):
        """
        The position inside the returned BoatState is in the frame of the vehicle's starting point
        Args:
            vehicle_name (str, optional): Name of vehicle
        Returns:
            BoatState:
        """
        state_raw = self.client.call('getBoatState', vehicle_name)
        return BoatState.from_msgpack(state_raw)
    def getBoatControls(self, vehicle_name=''):
        """
        Args:
            vehicle_name (str, optional): Name of vehicle
        Returns:
...
class MultiVehicleClient(object):
    ...
    def armDisarm(self, arm, vehicle_name=''):
        ...
        try:
            return self._multirotor.armDisarm(arm, vehicle_name)
        except Exception:
            try:
    ...
    def getCarState(self, vehicle_name=''):
        """Get car state. Use for car vehicles only."""
        return self._car.getCarState(vehicle_name)
    def getBoatState(self, vehicle_name=''):
