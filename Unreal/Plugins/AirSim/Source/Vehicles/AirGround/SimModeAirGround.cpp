...
bool ASimModeAirGround::isVehicleTypeSupported(const std::string& vehicle_type) const
{
    return AirSimSettings::isMultirotor(vehicle_type) || AirSimSettings::isCar(vehicle_type) ||
           AirSimSettings::isBoat(vehicle_type);
}
...
{
    if (!AirSimSettings::isBoat(vehicle_setting.vehicle_type) || !vehicle_setting.pawn_path.empty())
    ...
    APawn* spawned_pawn = this->GetWorld()->SpawnActor<ABoatPawn>(
