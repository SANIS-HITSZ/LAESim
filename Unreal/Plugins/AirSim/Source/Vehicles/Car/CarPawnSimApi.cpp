...
void CarPawnSimApi::updateCarControls()
{
    ...
    if (rc_data.is_initialized) {
        ...
        UAirBlueprintLib::LogMessageString("Control Mode: ", "API-only (no local keyboard fallback)", LogDebugLevel::Informational);
        current_controls_ = msr::airlib::CarApiBase::CarControls();
    }
