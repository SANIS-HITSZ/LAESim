...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kSimModeTypeAirGround = "AirGround";
    ...
    public: //methods
        ...
        {
            return vehicle_type == kVehicleTypePX4 || vehicle_type == kVehicleTypeArduCopterSolo ||
                   vehicle_type == kVehicleTypeArduCopter || vehicle_type == kVehicleTypeSimpleFlight;
        }
        ...
        static std::string inferSimModeFromVehicles(const Settings& settings_json)
        {
            ...
            for (const auto& key : keys) {
                Settings child;
                if (!vehicles_child.getChild(key, child))
                    continue;

                const auto vehicle_type = Utils::toLower(child.getString("VehicleType", ""));
    ...
    private:
        ...
        void loadCoreSimModeSettings(const Settings& settings_json, std::function<std::string(void)> simmode_getter)
        {
            ...
            if (simmode_name == "") {
                simmode_name = inferSimModeFromVehicles(settings_json);

                if (simmode_name == "" && simmode_getter)
            ...
            if (physics_engine_name == "") {
                if (simmode_name == kSimModeTypeMultirotor || simmode_name == kSimModeTypeAirGround)
                                                                    ...
                                                                    const CameraSetting& camera_defaults)
        {
            ...
            std::map<std::string, std::shared_ptr<SensorSetting>> vehicle_sensor_defaults = sensor_defaults;

            // In AirGround we host both cars and multirotors. Reusing the simmode-level default
            // sensor pack for every vehicle injects multirotor-only defaults like barometer and
            // magnetometer into cars, which is both semantically wrong and has caused runtime
            // instability in mixed scenes.
            if (simmode_name == kSimModeTypeAirGround && isCar(vehicle_type)) {
                ...
                for (const auto& sensor_pair : sensor_defaults) {
                    ...
                    if (sensor_type == SensorBase::SensorType::Gps ||
                        sensor_type == SensorBase::SensorType::Imu ||
                        sensor_type == SensorBase::SensorType::Distance ||
                        sensor_type == SensorBase::SensorType::Lidar) {
                        vehicle_sensor_defaults.emplace(sensor_pair.first, sensor_pair.second);
                    }
                }
            }
        ...
        void loadOtherSettings(const Settings& settings_json)
        {
            ...
            api_port_cv = settings_json.getInt("ApiServerPortCV", api_port);
